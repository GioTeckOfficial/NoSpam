...
client.on('message',message=>{
    ...
    fs.readFile(`./messages/${message.member.id}.json`,{encoding:'utf-8'},(err,data)=>{
        if(err) return fs.writeFileSync(`./messages/${message.member.id}.json`,`{"msg":"${message.content}"}`);
        let latest=message.content.toLowerCase();
        let oldest=JSON.parse(data).msg.toLowerCase();
